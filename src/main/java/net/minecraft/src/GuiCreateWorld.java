...
public class GuiCreateWorld extends GuiScreen {
    ...
    private boolean isCreative = false; // Creative modu için flag
    ...
    public void initGui() {
        ...
        // GameMode seçimi için bir düğme
        this.controlList.add(new GuiButton(2, this.width / 2 - 100, this.height / 4 + 72 + 12, "Game Mode: Survival"));
    ...
    protected void actionPerformed(GuiButton var1) {
        if (var1.enabled) {
            if (var1.id == 1) {
                ...
                // GameSettings içindeki creative modu ayarla
                GameSettings.isCreative = this.isCreative;

                this.mc.playerController = new PlayerControllerSP(this.mc);
                this.mc.startWorld(this.folderName, this.textboxWorldName.getText(), var2);
                this.mc.displayGuiScreen((GuiScreen) null);
            } else if (var1.id == 2) { // GameMode değiştir
                this.isCreative = !this.isCreative; // Durumu tersine çevir
                var1.displayString = "Game Mode: " + (this.isCreative ? "Creative" : "Survival");
            }
        }
    }
